...
remove <- function(x) {
  tmp <- as.matrix(x[-c(2,4,5,7,9,10:13,16,19,22,24,26,30),])
  return(tmp)
}
...
CV.SHC <- calc.CV(SHC[,-c(16:18)])
CV.SLC <- calc.CV(SLC[,-c(16:18)])

CV.AHC <- calc.CV(AHC[,-c(16:18)])
CV.ALC <- calc.CV(ALC[,-c(16:18)])

CV.S <- c(CV.SLC,CV.SHC)
CV.A <- c(CV.ALC,CV.AHC)

HCLC <- c(rep(1,15),rep(2,15))
...
num.clus(t(data3))
#Samples, 7 clusters chosen, even if silhouette coefficient for 9 clusters is higher, because no "0" cluster in 7 clusters
...
rownames(cl7)
cl8 <- PAM1[PAM1[,ncol(PAM1)]==8,]
rownames(cl8)
cl9 <- PAM1[PAM1[,ncol(PAM1)]==9,]
rownames(cl9)
cl10 <- PAM1[PAM1[,ncol(PAM1)]==10,]
rownames(cl10)