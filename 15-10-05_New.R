...
SynExp4ccmM <- SynExp4[,11:ncol(SynExp4)]
SynExp4 <- SynExp1[,1:10]
...
remove <- function(x) {
  tmp <- as.matrix(x[-c(2,4,5,10:13,16,19,22,24,26,30),])
  return(tmp)
}
# Dataset SynExp2 is completely removed, because of too many NAs
...
write.csv(Scaled, file="Complete_scaled_dataset.txt")

Transformed <- log10(Full)

write.csv(Transformed, file="Complete_logtransformed_dataset.txt")

par(oma = c(5,1,1,1))
boxplot(Scaled, las=2, cex=0.6, cex.axis=0.15)
abline(h = 0, col = "red")
...
par(oma = c(5,1,1,1))
boxplot(Transformed, las=2, cex=0.6, cex.axis=0.15)
abline(h = 0, col = "red")
...
par(oma = c(1,1,1,1))
samp <- hclust(as.dist(1-cor(Scaled, method="pearson")), method="average")
metab <- hclust(as.dist(1-cor(t(Scaled), method="pearson")), method="average")
heatmap(Scaled, main="Z scaled data", Rowv=as.dendrogram(metab), Colv=as.dendrogram(samp), scale="none", cexCol =0.15)
plot(samp, main="Z scaled data", hang=-1, cex=0.6)

par(oma = c(1,1,1,1))
samp1 <- hclust(as.dist(1-cor(Transformed, method="pearson")), method="average")
metab1 <- hclust(as.dist(1-cor(t(Transformed), method="pearson")), method="average")
heatmap(Transformed, main="log transformed data", Rowv=as.dendrogram(metab1), Colv=as.dendrogram(samp1), scale="none", cexCol =0.15)
plot(samp1, main="log transformed data", hang=-1, cex=0.6)
...
###Select number of clusters with the highest average silhouette coefficient
###here k=4 and k=2 for Scaled
###and k=2 and k=2 for Transformed


#Run pam for samples
Trans.clu.samp <- pam(t(Transformed), k=2)
Scale.clu.samp <- pam(t(Scaled), k=2)

#Run pam for metabolites
Trans.clu.metab <- pam(Transformed, k=2)
Scale.clu.metab <- pam(Scaled, k=4)
...
heatmap(Scaled, Rowv=as.dendrogram(metab), Colv=as.dendrogram(samp), col=color(), scale="none", main="Pearson correlation, average clustering (Z scaled data)") 
heatmap(Transformed, Rowv=as.dendrogram(metab1), Colv=as.dendrogram(samp1), col=color(), scale="none", main="Pearson correlation, average clustering (log transformed data)") 

#Cut tree according to pam clustering results "Scaled"
pamsample <- sample(rainbow(2)) 
pamsample <- pamsample[as.vector(Scale.clu.samp$clustering)]
pammetab <- sample(rainbow(4))
pammetab <- pammetab[as.vector(Scale.clu.metab$clustering)]
par(oma=c(7,2,2,5))
heatmap(Scaled, Rowv=as.dendrogram(metab), Colv=as.dendrogram(samp), col=color(), scale="none", RowSideColors=pammetab, ColSideColors=pamsample, main="Biclustering (pearson, average, (Z scaled data))")

#Cut tree according to pam clustering results "Transformed"
pamsample1 <- sample(rainbow(2)) 
pamsample1 <- pamsample1[as.vector(Trans.clu.samp$clustering)]
pammetab1 <- sample(rainbow(2))
pammetab1 <- pammetab1[as.vector(Trans.clu.metab$clustering)]
par(oma=c(5,2,2,5))
heatmap(Transformed, Rowv=as.dendrogram(metab1), Colv=as.dendrogram(samp1), col=color(), scale="none", RowSideColors=pammetab1, ColSideColors=pamsample1, main="Biclustering (pearson, average, (log transformed data))")

# Read out PAM clusters --------------------------------------

S.PAM <- cbind(t(Scaled), Scale.clu.samp$clustering)
Tr.PAM <- cbind(t(Transformed), Trans.clu.samp$clustering)
...
cond <- c(rep("SHC",4), rep("S3LC",4), rep("S24LC",2))
cond.ident <- c(rep(1,4), rep(2,4), rep(3,2))
...
AHC <- rbind(subset(Full.2, call.cond==4), subset(Full.2, call.cond==22), subset(Full.2, call.cond==23))
...
CV.SHC <- calc.CV(SHC[,-c(18:20)])
CV.SLC <- calc.CV(SLC[,-c(18:20)])

CV.AHC <- calc.CV(AHC[,-c(18:20)])
CV.ALC <- calc.CV(ALC[,-c(18:20)])
...
RMlist <- list(exp1HCMat, exp1LC1Mat, exp1LC2Mat, exp2HCMat, exp2LC1Mat, exp2LC2Mat, exp3HCMat, exp3LC1Mat, exp3LC2Mat, exp4HCMat, exp4LC1Mat, exp4LC2Mat, exp4LC3Mat, exp5HCMat, exp5LC1Mat, exp5LC2Mat, exp5LC3Mat, exp6NC1Mat, exp6NC2Mat, exp6NC3Mat, exp6NC4Mat, exp6NC5Mat, exp6NC6Mat, exp6NC7Mat, exp6VC1Mat, exp6VC2Mat, exp6VC3Mat, exp6VC4Mat, exp6VC5Mat, exp6VC6Mat, exp6VC7Mat, exp7NC1Mat, exp7NC2Mat, exp7NC3Mat, exp7NC4Mat, exp7NC5Mat, exp7NC6Mat, exp7NC7Mat, exp7HC1Mat, exp7HC2Mat, exp7HC3Mat, exp7HC4Mat, exp7HC5Mat, exp7HC6Mat, exp7HC7Mat)
...
names <- c("Syn exp1 HC", "Syn exp1 3h LC", "Syn exp1 24h LC", "Syn exp2 HC", "Syn exp2 3h LC", "Syn exp2 24h LC", "Syn exp3 HC", "Syn exp3 3h LC", "Syn exp3 24h LC", "Ara exp4 light HC", "Ara exp4 light 1d LC", "Ara exp4 light 3d LC", "Ara exp4 light 5d LC", "Ara exp5 dark HC", "Ara exp5 dark 1d LC", "Ara exp5 3d LC", "Ara exp5 dark 5d LC", "Ara exp6 2h NC", "Ara exp6 4h NC", "Ara exp6 6h NC", "Ara exp6 8h NC", "Ara exp6 10h NC", "Ara exp6 12h NC", "Ara exp6 24h NC", "Ara exp6 2h VC", "Ara exp6 4h VC", "Ara exp6 6h VC", "Ara exp6 8h VC", "Ara exp6 10h VC", "Ara exp6 12h VC", "Ara exp6 24h VC", "Ara exp7 2h NC", "Ara exp7 4h NC", "Ara exp7 6h NC", "Ara exp7 8h NC", "Ara exp7 10h NC", "Ara exp7 12h NC", "Ara exp7 24h NC", "Ara exp7 2h HC", "Ara exp7 4h HC", "Ara exp7 6h HC", "Ara exp7 8h HC", "Ara exp7 10h HC", "Ara exp7 12h HC", "Ara exp7 24h HC")
...
par(oma=c(5,1,1,1))
heatmap(rv_matrix, scale="none")

num.clus(rv_matrix)
num.clus(rv_matrix1)

#Run pam
RVclu <- pam(rv_matrix, k=10)
RVclu1 <- pam(rv_matrix1, k=7)

RVcolors <- sample(rainbow(10)) 
RVcolors <- RVcolors[as.vector(RVclu$clustering)]

exp <- hclust(as.dist(1-cor(rv_matrix, method="pearson")), method="average")
par(oma = c(1,1,1,1))
plot(exp, main="RV coefficients between metabolite ratio matrices", hang=-1, cex=1)
par(oma = c(5,1,1,5))
heatmap(rv_matrix, col=color(), scale="none", Rowv=as.dendrogram(exp), Colv="Rowv", ColSideColors=RVcolors, RowSideColors=RVcolors, main="PAM clustering of RV coefficients between metabolite ratio matrices (k=10)")
...
pear1 <- hclust(as.dist(1-cor(rv_matrix1, method="pearson")), method="average")
par(oma = c(1,1,1,1))
plot(pear1, main="RV coefficients between metabolite ratio matrices", hang=-1, cex=1)

par(oma = c(5,1,1,5))
heatmap(rv_matrix1, col=color(), scale="none", Rowv=as.dendrogram(pear1), Colv="Rowv", ColSideColors=RVcolors, RowSideColors=RVcolors, main="PAM clustering of RV coefficients between metabolite ratio matrices (k=7)")

PAM <- cbind(rv_matrix, RVclu$clustering)
...
plot(exp, main="RV coefficients between metabolite ratio matrices", hang=-1, cex=1)
par(oma = c(5,1,1,5))
heatmap(rv_matrix, col=color(), scale="none", Rowv=as.dendrogram(exp), Colv="Rowv", ColSideColors=RVcolors, RowSideColors=RVcolors, main="PAM clustering of RV coefficients between metabolite ratio matrices (k=4)")
